...
class ConvLoRA_split(nn.Module, LoRALayer):
    def __init__(self, conv_module, in_channels, out_channels, kernel_size, r=0, lora_alpha=1, lora_dropout=0., merge_weights=True, information='fisher',dropout_coef=0.2,**kwargs):
        ...
        if r > 0:
            ...
            out_ch = w_shape[0]
            in_features = torch.prod(torch.tensor(w_shape[1:])).item()
            self.lora_B = nn.Parameter(self.conv.weight.new_zeros((out_ch, r)))
            self.lora_A = nn.Parameter(self.conv.weight.new_zeros((r, int(in_features))))
            self.scaling = self.lora_alpha / self.r
    ...
    def forward(self, x):
        if self.r > 0 and not self.merged:
            lora_weight = (self.lora_B @ self.lora_A).view(self.conv.weight.shape)
            return self.conv._conv_forward(
                x, 
                self.conv.weight + lora_weight * self.scaling, 
                self.conv.bias
            )
        return self.conv(x)

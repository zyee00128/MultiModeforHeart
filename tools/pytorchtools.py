...
class EarlyStopping:
    ...
    def __init__(self,patience=7, verbose=False,dataset_name='CODE15_Pretrain',delta=0,args=None,path = '/pretrained_checkpoint'):
        """
        Args:
        ...
        self.save_dir = os.path.join(self.args.root, clean_path)
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        # # os.chdir(self.path)
        save_path = os.path.join(self.save_dir, self.dataset_name + '_checkpoint.pt')
        
        is_lora = False
        if hasattr(self.args, 'ranklist'):
            # 检查 ranklist 是否包含 'lora' 字符串
            if isinstance(self.args.ranklist, str) and 'lora' in self.args.ranklist:
                is_lora = True

        if is_lora:
            # print(f"Saving LoRA/Partial weights to {save_path}")
            saving_lora_checkpoint(model, save_path)
        else:
            # print(f"Saving FULL model weights to {save_path}")
